...
状态: 部分完成（09e63c6 后端达标 / 469ca1c 前端未达标）
...
1. **后端 ✅ 达标**。超标的不止条目里写的 persistence.py，实测有四个，全部拆完：
   ...
   `.commands` 撞上新建同名子模块。
2. **前端 ❌ 未达标**。三个组件抽出无状态部分后是 879 / 783 / 901，仍超 700。
   剩下的体量是 600+ 行 useState/useEffect/useCallback，靠继续搬 JSX 补不上，
   必须抽成 `useXxxWorkspace` 自定义 hook（一次要动 ~40 个返回值）。**这是 Q10 的
   唯一剩余工作。**

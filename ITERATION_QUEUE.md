...
## Q39 · Agent 检索范围与联网切换闭环

状态: 代码完成，定向门禁通过，真实页面与 Provider 由用户手动验收
对应: 2026-08-04 用户反馈“仅网址库”仍出现库外 LLM 推荐，要求未命中时可在同一上下文切换联网搜索

交付与边界：

- `仅网址库` 现在是严格闭世界：后端不挂 `web_search`，`search_library` 无命中时返回可识别的
  `can_offer_online` 信号；`present_website_recommendations` 拒绝所有未解析为当前账号 Site 的库外 URL，
  不再允许模型记忆生成可点击外部卡片。
- 前端把该信号渲染为“开启联网搜索”按钮。点击后同步切换当前 scope，复用最近一条用户问题和现有
  `conversationId` 重新发送，不新建会话、不丢上下文；未配置搜索 Provider 时由联网回合如实提示配置缺失。
- `允许联网` 仍先检索网址库；只有结果不足或用户明确需要实时资料时才交给 `web_search`。每轮真实搜索
  返回的 URL 会登记在受信集合中，外部推荐必须命中该集合；仅配置 Provider 或模型凭记忆写出的 URL 不得
  冒充真实 Provider 来源。
- 旧会话中的 `llm推荐` 兼容显示为“模型知识（未实时验证）”，不再硬写“来自网络”；新回合不再生成该
  来源的外部卡片。控件补充范围说明 tooltip，Provider 设置文案明确“不会调用外部搜索服务”与模型调用的区别。

定向测试：后端覆盖 collection/online 外部 URL 门禁、真实搜索 URL 登记与切换提示；前端覆盖未命中联网
信号、同会话重试钩子与按钮渲染。按用户要求不运行 Playwright、不调用真实搜索 Provider。

用户手动验收：在仅网址库下询问库内无匹配主题，回答只能说明未找到并出现“开启联网搜索”；点击后顶部切为
允许联网、同一会话追加原问题、出现真实 Provider 阶段与外部结果。联网模式下先准备一个库内命中和一个库内
无命中问题，分别确认只返回库内结果与缺口触发真实搜索；未配置 Provider 时不能伪造外部网址。

---

## Q40 · Agent 网址库完整结果与推荐排序分页

状态: 代码完成，定向门禁通过，真实页面由用户手动验收
对应: 2026-08-04 用户反馈“105/22 条检索结果最终只展示 4 条，没有翻页”

交付与边界：

- `search_library` 对未指定数量的推荐默认 `include_all=true`，服务端冻结全部账号内命中结果，模型只接收
  有界预览和 `result_set_id`，不会把全量清单塞回模型上下文。
- 完整结果使用不调用 LLM、Embedding 或联网的确定性相关度排序：名称精确匹配优先，其次 URL/分词命中，
  最后按规范化名称和 `site_id` 稳定排序，保证分页顺序不会因数据库默认顺序漂移。
- `present_website_recommendations` 收到 `result_set_id` 时原样交付完整 artifact；模型若误把本轮预览作为
  `items` 传回，仅当本轮最新检索仍是全量时，才从最近且覆盖预览条目的完整集合自动恢复，不扩大显式
  `include_all=false` 精选结果。
- 前端现有 Agent 结果组件继续按 12 条/页渲染数字页码、上一页/下一页及首尾边界；这次修复重点保证它实际拿到
  全量数组，而不是只对 4 条模型精选卡片分页。

定向测试与门禁：后端 Agent 工具/runner 48/48、定向 Ruff、前端 TypeScript、定向 ESLint、完整 Node
测试 217/217、`git diff --check` 通过；按用户要求未运行 Playwright、未调用真实搜索 Provider。

用户手动验收：发送“把库里的所有 AI 类网站都发给我”，确认标题总数与卡片一致，105 条显示 9 页、22 条显示
2 页，按推荐程度排序，数字页码可直达且无重复/遗漏；刷新或打开历史对话后仍保留完整分页。明确“精选 4 个”
时仍只显示 4 个，不被全量兜底扩大。

---

## Q41 · Agent 来源信任、URL 安全与持久化容量硬化

状态: 代码与自动化门禁完成；前端 222/222、后端 708/708、完整 lint/typecheck、Next.js 静态页 12/12 与根级 `pnpm check` 通过，真实页面由用户手动验收
对应: 2026-08-30 继续排查 Q39-Q40 后的来源伪造、URL 解析差异、显式数量和回合持久化容量边界

交付与边界：

- 新回合使用推荐 manifest v3；服务端和前端继续只读兼容合法 v2 历史，不把旧数据重写成 v3。
  v3 Assistant 正文与 reasoning 中的外部 HTTP(S) 链接，必须按规范化后的 scheme、host、path 和 query
  精确命中同一条 Assistant 消息里的受信 `source-url`；fragment 只能用于该受信页面内定位，不能扩大
  到另一地址。旧 `llm推荐` 继续显示为“模型知识（未实时验证）”，其 URL 和 favicon 仅经过前端启发式
  兼容过滤：凭据、私网/本机、敏感 query/fragment（含 hash-router 参数）会变为不可点击或移除。
- `仅网址库` 是默认范围；只有用户显式切到 `允许联网` 才挂载搜索能力，而且仍必须先查网址库。
  `web_search.query` 必须复用本轮最近一次网址库检索的完全相同规范化 query。服务端还会阻止模型把
  最近站内结果中的名称、URL、摘要、介绍、分类或标签片段擅自拼进外发查询；这只是降低误外发的
  heuristic，不是数据防泄漏系统（DLP），不能据此承诺识别所有私有或敏感信息。
- Provider 返回 URL 在搜索工具和 LangGraph runner 两层重新 canonicalize、过滤、去重，只允许无凭据、
  无敏感参数的公开 HTTP(S) 地址。私网/本机目标、反斜杠或畸形 URL、畸形 IPv6、单标签主机、私有/
  保留后缀（含 `home.arpa`）、组播地址、编码 authority，以及十进制单整数、十六进制、八进制或缩写点分形式等非规范数字 IPv4
  均拒绝，不能进入模型可见工具结果、实时 `source-url`、持久历史或同回合重放。已持久化的旧
  `web_search` 来源会在重放时重新校验；孤立 surrogate 和非有限 JSON 数值 fail closed，Provider 的
  流式 text/reasoning 在入流和入库前统一做 UTF-8 安全投影。
- 推荐 artifact 的单回合 JSON 上限为 256 KiB；超过后明确返回 `result_set_too_large`，要求缩小关键词、
  分类或星标范围，不把截断清单冒充完整结果。持久 `sources` 总预算为 384 KiB，持久 `source-url`
  parts 预算为 64 KiB；过大的普通工具结果改存 `persisted_tool_result_truncated` 摘要。可执行的
  `propose_*` Space/写入草稿优先于可重建的搜索和推荐结果保留。最终推荐与 `source-url` 只有在持久化
  接纳后才发送，因此这些结果的 live/history/replay 一致；较早已实时显示的可重建只读中间结果在聚合
  超限时可能只以摘要进入历史/重放，不承诺整个 live 序列逐事件重现。
- 用户明确要求 N 条时，服务端不信任模型的 `include_all`：先按确定性查询相关度排序，在命中足够时冻结
  恰好前 N 条并返回 `selected_count` 与 `result_set_id`，不足 N 时如实返回实际命中数；模型只看到有界
  预览。未指定数量或要求全量时同样按确定性查询相关度冻结；只有没有明确数量的主观精选
  `include_all=false` 继续按 `updated desc` 取有限候选。Agent 这几条路径都不使用 Embedding。
- 105 条/9 页与 22 条/2 页只是当前分页回归样例，不证明任意规模或 100,000 条 Agent 结果可交付。
  当前没有任意规模的 Agent 结果集分页 API；完整 artifact 超过 256 KiB 会按上述合同拒绝。

自动化证据：前端完整 Node 测试 **222/222**、受影响后端六文件 **127/127**、后端完整 **708/708**；
完整 Ruff/ESLint、TypeScript、Next.js 生产构建（静态页 **12/12**）及根级 `pnpm check` 已通过。未调用真实
搜索 Provider，未运行真实浏览器/Playwright，也未接触真实书签或 `.data`。

用户手动验收：在默认“仅网址库”确认库外链接不可点击且无命中时出现联网切换；显式切换后核对同一
query 先查库再联网。分别请求“精选 15 个 AI 网站”和未指定数量的完整结果，确认前者命中足够时恰为
15 条并可刷新恢复，后者仍按实际结果分页。用受控假 Provider 返回私网、带凭据、敏感参数、编码数字 IP、
反斜杠和一个正常公网 URL，确认只有正常地址可进入来源、历史和重放；这项不能用真实 Provider 数据代替。
...
当前证据基线：前端完整 222/222、Q41 受影响后端六文件 127/127、后端完整 708/708；完整 Ruff/ESLint、
TypeScript、Next.js 生产构建（静态页 12/12）与根级 `pnpm check` 已通过。本轮新增功能必须带测试，
已确认的基线只能涨不能降。

...
// 下拉里代表「切回手填」的哨兵值。用一个不可能与真实模型 ID 冲突的串。
const MANUAL_MODEL_OPTION = "\u0000manual";
...
export function ProviderForm({
  ...
  const [draft, setDraft] = useState<ProviderDraft>(() => {
    if (!config) return createProviderDraft(registry.length === 1 ? registry[0] : null);
    const initial = editProviderDraft(config);
    const configDefinition = registry.find((item) => item.provider === config.provider);
    return configDefinition?.fixedBaseUrl
      ? { ...initial, baseUrl: configDefinition.defaultBaseUrl ?? "" }
      : initial;
  });
  ...
  // 拉到列表后默认走下拉选择；用户想填目录里还没有的新模型时切回手填。
  ...
  const selectVendor = (item: ProviderRegistryItem) => {
    if (item.provider === draft.provider) return;
    const previous = registry.find((entry) => entry.provider === draft.provider) ?? null;
    // Base URL 用新厂商的官方地址预填。只有当前值是空的、或就是上一个厂商的
    // 默认值时才覆盖——用户手填过的代理地址不能被换厂商这个动作抹掉。
    const untouchedBaseUrl =
      !draft.baseUrl.trim() || draft.baseUrl.trim() === (previous?.defaultBaseUrl ?? "\u0000");
    const nextBaseUrl = item.fixedBaseUrl || untouchedBaseUrl
      ? item.defaultBaseUrl ?? ""
      : draft.baseUrl;
    // Key 只属于当前厂商。换厂商时必须同时清掉 DOM 明文和 React 中的意图状态，
    // 防止把上一家的 Key 误发给下一家，或让已卸载的输入框留下虚假的 filled 状态。
    resetSecretInput();
    patchDraft(
      {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editing && config && onUpdate) {
        // 固定地址在后端以 null 存储、运行时回落到注册表默认值。比较编辑草稿时也要
        // 使用同一语义，否则只打开弹窗再保存就会被误判成一次 Base URL 修改。
        const comparisonConfig = definition?.fixedBaseUrl
  ...
  const testsSearch = kind === "search";
  const hidesSecretInput = definition?.fixedBaseUrl === true && !definition.secretRequired;
  const submitting = busy;

  // 已保存配置的模型名可能不在这次拉到的目录里（厂商下架了、或当初手填的），
  // 仍要出现在下拉里，否则一打开编辑框就显示成「请选择模型」。
  const modelOptions =
    ...
    <form className="provider-form" onSubmit={(event) => void handleSubmit(event)} noValidate>
              ...
              return (
                ...
                >
                  ...
                  <span className="provider-registry-item-note">
                    ...
                    {!item.searchBulkSupported ? " · 仅低频 · 不支持批量" : ""}
                  </span>
                </button>
              );
            })}
      ...
      {definition && (
        definition.usageNotice && (
          <p className="provider-test-result provider-form-field--full" data-status="neutral" role="note">
            {definition.usageNotice}
          </p>
        )
      )}

      {definition && !definition.fixedBaseUrl && (
        <div className="provider-form-field provider-form-field--full">
      ...
      <div className="provider-form-actions">
        {definition?.connectionTestSupported && (
          <button
            ...
            title={
              testsSearch
                ? "向服务商执行一次最小测试搜索，可能消耗一次查询额度"
                : "用当前 API Key 向服务商拉取可用模型，同时验证连接"
            }
          >
            {testing ? <Spinner size={14} /> : <PlugZap aria-hidden="true" />}
            {testing
              ? testsSearch ? "正在测试搜索" : "正在获取"
              : testsSearch ? "测试搜索（可能消耗额度）" : "获取模型列表"}
          </button>
        )}
        <button
          className="provider-btn provider-btn-secondary"

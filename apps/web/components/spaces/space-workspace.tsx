...
type MemberMove = "top" | "up" | "down" | "bottom";

const EMPTY_SPACE_PAGE: SpacePageState = {
  items: [],
  nextCursor: null,
  totalCount: 0,
  loadingMore: false,
...
function appendUniqueSpaces(current: Space[], incoming: Space[]): Space[] {
  const ids = new Set(current.map((space) => space.id));
  return [...current, ...incoming.filter((space) => !ids.has(space.id))];
}

function appendUniqueMembers(current: SpaceMember[], incoming: SpaceMember[]): SpaceMember[] {
...
function isSpaceStatus(error: unknown, status: number): error is SpaceApiError {
  return error instanceof SpaceApiError && error.status === status;
}

function isSpaceCode(error: unknown, code: string): error is SpaceApiError {

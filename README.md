...
[![status](https://img.shields.io/badge/status-Q36%20%C2%B7%20local--first%20MVP-orange)](./PROGRESS.md)
...
| **用 Agent 查找和整理** | Agent 默认使用当前账号的网址库；回答支持流式 Markdown、经服务端筛选的来源链接、工具时间线、推理折叠、停止与历史恢复。写操作先生成草稿，只有用户确认后才落库。 |
...
| **完整的生成式 UI 流协议** | 后端实现 AI SDK UI Message Stream v1，统一传输文本、reasoning、工具调用/结果、可信 `source-url`、Provider usage、TTFT 与总耗时；响应禁止代理缓冲。 | 前端得到的不是一段字符串，而是一条可恢复、可观测、可渲染为卡片与时间线的结构化事件流。 |
...
截至 2026-07-31 · Q36，自动化证据如下；新增功能必须带测试，基线只能涨不能降：

| 范围 | 结果 | 说明 |
| --- | --- | --- |
| 前端完整测试 | **212 / 212** | 当前完整基线。 |
| 后端完整测试 | **655 / 655** | 最近一次完整通过基线来自 Q32。 |
| Q35 后端聚焦回归 | **88 / 88** | 覆盖单站分析阶段、持久补全等本轮相关链路。 |
| Alembic 迁移往返 | **17 / 17** | 开发库已确认位于 `20260731_0019 (head)`。 |
| Q36 静态与构建门禁 | **通过** | TypeScript、全前端 ESLint、Next.js 生产构建与 `git diff --check`。 |

> [!NOTE]
> Q36 时后端全量 pytest 在约 184 秒被 shell / 工具超时终止，终止前未见失败；这既不是测试失败，
> 也不能写成“当前后端全量 655 / 655 通过”，所以上表明确保留最近一次完整基线与当前聚焦结果的区别。
...
**最新快照：2026-07-31 · Q36。** Provider 配置、Agent 增改查、书签导入、网页抓取、批量入库、
...
网址库的“相关度”排序会使用可选 Embedding，但 Agent 的站内检索当前仍使用关键词路径；向量层最多扫描
20,000 条记录，定位于个人规模而非海量数据。新站写库本身是持久的，但当分析队列完全占满或 API 在执行前
退出时，本次即时 LLM 分析意图尚不能保证恢复；只有首页显式启动的全库补全任务具备持久恢复语义。

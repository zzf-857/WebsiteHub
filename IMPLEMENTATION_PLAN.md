...
#### Phase 2B checkpoint（2026-07-26）

后端逻辑已完成：

- Space 与 Space 成员关系的 Alembic migration、账号级复合外键和账号内名称唯一约束
- Space 列表、创建、详情、重命名、删除预览、删除及成员新增、移除和批量移动排序
- 同一 Site 可属于多个 Space；删除 Space 只解除关系，删除 Site 会推进所有相关 Space 版本
...
当前进展（2026-07-26）：账号隔离的 Conversation/Message 表、复合账号外键、CAS、幂等消息、来源与 artifact JSON 边界、按 UTC 偏移的日期分组、重命名/删除和服务端 Slash Command 注册表已完成。Python 已实现 AI SDK v7 UI Message Stream v1 编码、typed data、finish/error/abort、部分消息与 golden fixture，并提供 fail-closed 的 `/api/agent/chat` 路由合同。真实 Agent Runner、Provider 调用、流式消息持久化接线和 `useChat` 前端仍属于下一阶段，不得把安全错误流视为真实回答。
...
> **状态快照已外移到 `PROGRESS.md`**，本节不再重复，避免两份状态互相打架。
> 这一节曾经写着「Phase 4 是当前最大阻塞 / `/settings/providers` 是纯占位页」，
> 而那些在 2026-07-26 当天就做完了——过期的快照比没有快照更有害，
> 它会让接手的人去重做已完成的事。
>
> - 调度依据：`ITERATION_QUEUE.md`（唯一入口，从上往下取第一个「待做」）
> - 当前能跑什么 / 不能跑什么 / 刻意没做什么：`PROGRESS.md`
> - 本机环境（固定账号、库位置、测试素材）：`LOCAL_DEV.md`（不进 Git）
>
> 本文件其余章节保留架构与工程决策，那些不随迭代变化。

...
Provider 配置、Agent 增改查、书签导入三条链路**已端到端跑通并实测**；
抓取、批量入库、自定义排序、RAG、视觉收尾尚未开工。

测试基线：**前端 124 / 后端 341**。只能涨不能降。
...
| 网站抓取（Q4） | 抓取子系统整体不存在，`analysis_status` 全后端零写入点 |
...
2. 之后按 `ITERATION_QUEUE.md` 顺序走 Q4 → Q9。

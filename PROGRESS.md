...
最后更新：2026-07-28 · Q16 元数据补全闭环代码完成，待用户手动验收
...
> Q16 起，打开首页或资料库会自动、低并发地开始扫尾历史 `not_analyzed` 网站。不要反复点击
> “补全网站信息”堆叠任务；让后台持续运行并观察可见卡片是否自行更新即可。
...
| Q14-Q16 页面功能未手动验收 | 本轮只完成代码静态复核；未运行测试、构建或页面检查，上表及真实 Chrome 清单需用户逐项检查 |
...
**Q1-Q16 的代码项已完成。以下是尚存的验收与优化缺口，按影响排序：**

1. **用户用真实 Chrome 验收 Q14-Q16 页面能力**：Provider/语义索引分区、真实 favicon、
   紧凑预览放大、站内详情、Agent 流式富文本、100 条批量删除、资料库静默分页以及历史元数据
   自动扫尾。打开一次首页或资料库后让后台低并发运行，核对可见卡片是否自行更新。

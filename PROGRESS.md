...
最后更新：2026-07-27 · Q13 二次安全收口与真实 Provider / Chrome 验收已完成
...
此前 100% 不生效：花掉 token、报告成功、库里一行没变）、补完多处断链，并在二次收口中
补齐完整快照、写锁、断连取消、并发/重试上界与迁移回填。真实 Provider 和 Chrome 均已验收，
详见 `ITERATION_QUEUE.md` 的 Q13 条目。**队列再次清空**，下一批做什么需要用户排期。

测试基线：**前端 147 / 后端 459**。只能涨不能降。
...
1. **两处 Observer 真实浏览器验证**：Q6 拖拽持久化与 Q9 的 `data-stuck` /
   `data-compact` 吸顶触发时机。代码已完成并有单测，但内嵌面板里 Observer 回调
   被挂起（见下方环境陷阱第 4 条），需在真实 Chrome 里配合操作验证。
2. **分类图标选择器**：后端 `icon` 字段与契约层都已就绪，只缺一块让用户手选的 UI。
3. **Agent 聊天里确认书签导入**：`propose_bookmark_import` 后端已出草稿，
   前端缺契约与确认通道；`prompt.py` 目前向模型承诺了这个能力，属虚假宣传，
   要么补前端要么改提示。
4. **大契约文件拆分**：`lib/agent-contract.ts` 已超 750 行，超出前端 700 行约定。
